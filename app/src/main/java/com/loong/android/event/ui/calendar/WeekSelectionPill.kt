...
private fun getOffsetAndSize(
    ...
    val scaledPercentage = if (selectedPercentage >= endPercentage) {
        ...
        normalize(
            selectedPercentage, startPercentage, endPercentage
        )
    }
    ...
    var totalSize =
        (scaledSelectedNumberDays * widthPerDayPx) + (leftSize + rightSize) * scaledPercentage
    ...
    val offset = if (state.animateDirection?.isBackwards() == true) {
...
private fun normalize(
    x: Float, inMin: Float, inMax: Float, outMin: Float = 0f, outMax: Float = 1f
): Float {
